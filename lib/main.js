...
for(c=0; c<ColumnCount*2-1; c++) {
    ...
    for(r=0; r<rows; r++) {
        tempWalls[c][r] = { x: 0, y: 0 , active: false};
    }
}
*/
...
function mousedownHandler(e) {
    //TODO Drag a pawn and Activing wall depending which state is active
}

function mouseOverHandler(e) {
    //TODO Highlight temp walls. Loop walls list and temp highlight it if not active
...
function drawTempWalls(){
    for(c=0; c<ColumnCount*2-1; c++) {
        ...
        for(r=0; r<rows; r++) {
            ...
            if (tempWalls[c][r].active && (walls[c][r].active == false)){
                tempWalls[c][r].x = wX;
                tempWalls[c][r].y = wY;
                ctx.beginPath();
                if ((c % 2) == 1) {
